...
const historicalShlLiveEnabled =
  process.env.TRUSTCARE_PORTAL_LIVE_HISTORICAL_SHL_TEST === "1";
const historicalShlOnly =
  process.env.TRUSTCARE_PORTAL_LIVE_HISTORICAL_SHL_ONLY === "1";
...
describe.skipIf(!liveEnabled)("live Portal Wallet binding and sync", () => {
  it.each(linkedUsernames)(
    "%s completes test-login -> binding -> DPoP session -> sync",
    async (username) => {
      ...
      const trackingFetch: typeof fetch = async (request, init) => {
        ...
        if (String(request) === contracts.discovery.endpoints.credentialSync) {
          syncResponseMeta = {
        ...
        if (change.type === "credential.upsert") {
          ...
          if (!prepared.document) {
            const subject = record(
              change.credential.credentialData?.credentialSubject,
            );
            ...
            const issuer = issuers.find(
              (candidate) =>
                candidate.issuerDid === change.credential.issuerDid,
            );
            const verification =
              issuer &&
      ...
      expect(
        ack.summary.applied + ack.summary.archived + ack.summary.rejected,
      ).toBe(page.changes.length);
...
type LiveIssuers = Awaited<ReturnType<typeof resolveAllPortalHospitalIssuers>>;
type LiveSyncPage = Awaited<
  ReturnType<WalletExchangeV2Client["syncCredentials"]>
>;
type LiveGraphState = ReturnType<typeof createWalletClinicalDocumentGraphState>;
...
async function runPortalQrClosure(input: {
  ...
  const source = input.documents.find(
    (document) =>
      ...
      ["verified", "issuer_signed_untrusted"].includes(document.trust.state) &&
  ...
  try {
    ...
    oid4vciReplay =
      reason instanceof PortalInteroperabilityProblemError ? reason : undefined;
  }
  ...
  const publicConsentRef = `urn:trustcare:consent:public-vp:${globalThis.crypto.randomUUID()}`;
  ...
  console.info("Live Wallet QR closure", {
    ...
    publicEvidenceRequestId: publicPublication.verificationEvidence.requestId,
...
async function runPortalP0Completion(input: {
  ...
  if (historicalShlLiveEnabled && input.username === linkedUsernames[0]) {
    await expectWalletProblem(
      () => input.exchange.getCredentialRequestStatus("453"),
      [410],
      "shl_holder_authorization_expired",
    );
    if (historicalShlOnly) return;
  }
  ...
  if (credentialRequestStatus.nextAction !== "sync_credentials") {
    const maker = await portalStaffLogin("demo-nurse-001", "issuer_maker");
    const checker = await portalStaffLogin("demo-doctor-001", "issuer_checker");
    ...
    const queueItem = findWalletQueueItem({
      ...
      itemRequestIds: credentialRequestStatus.items.map(
        (item) => item.requestId,
      ),
    });
  ...
  console.info("Live Wallet fresh Certified SHL request", {
    credentialRequestId: shlRequest.requestId,
    shlPackageId,
    authorizationExpiresAt: preparedShl.packageBinding.accessPolicy.expiresAt,
    idempotencyKey: shlRequestKey,
  });
  ...
  if (shlRequestStatus.nextAction !== "sync_credentials") {
    ...
    expect(claimed.nextStep).toBe("organization_approval_required");
    const approvalTaskId = nonEmptyString(
      claimed.approvalTaskId,
      "Portal SHL organization approval task ID",
    );
    const approvalDescriptor = shlApprovalDescriptor(claimed.approvalTask);
    expect(approvalTaskId).toBe(approvalDescriptor.taskId);
    expect(approvalDescriptor.assignedRole).toBe("organization_approver");
    console.info("Live Wallet Certified SHL approval descriptor", {
      credentialRequestId: shlRequest.requestId,
      issuanceRequestId,
      authorizationExpiresAt: preparedShl.packageBinding.accessPolicy.expiresAt,
      ...approvalDescriptor,
    });
    const organizationApproval = await approveShlOrganizationTask({
      hospitalId: maker.user.hospitalId,
      descriptor: approvalDescriptor,
      runId,
    });
    expect(organizationApproval).toMatchObject({
      state: "approved",
      approved: true,
    });
    const certifiedShlIssuance = record(
      organizationApproval.certifiedShlIssuance,
    );
    expect(certifiedShlIssuance).toMatchObject({
      issued: true,
      nextStep: "wallet_credential_sync",
    });
    const issuedManifestCredential = record(certifiedShlIssuance.credential);
    nonEmptyString(
      issuedManifestCredential.jwt,
      "decideTask certifiedShlIssuance.credential.jwt",
    );
    console.info("Live Wallet Certified SHL KMS finalization", {
      issued: certifiedShlIssuance.issued,
      nextStep: certifiedShlIssuance.nextStep,
      credentialId:
        issuedManifestCredential.credentialId ??
        issuedManifestCredential.id ??
        certifiedShlIssuance.credentialId,
    });
  ...
  const manifestClaims = record(record(manifestPayload.credentialSubject).data);
  ...
  expect(manifestClaims).toMatchObject({
    purpose,
    context: "opd_visit",
    consentRef,
  });
  expect(record(manifestClaims.hospital).did).toBe(tcc.issuerDid);
  ...
  if (input.username === linkedUsernames[0]) {
    ...
    const wrongPurposeVp = await resignHolderVpPurpose({
      identity: input.holder,
      payload: holderVp.payload,
    ...
    await expectWalletProblem(
      () =>
        input.exchange.associateShlPresentation(
          shlId,
          {
            clientAssociationId: `negative-purpose-${runId}`,
            consentRef,
            holderVpJwt: wrongPurposeVp,
          },
          `negative-purpose-${runId}`,
        ),
      [422],
      "wrong_shl_purpose",
      "presentation_purpose_mismatch",
    );
    const transportPurposeVp = await resignHolderVpPurpose({
      identity: input.holder,
      payload: holderVp.payload,
    ...
    await expectWalletProblem(
      () =>
        input.exchange.associateShlPresentation(
          shlId,
          {
            clientAssociationId: `negative-transport-purpose-${runId}`,
            consentRef,
            holderVpJwt: transportPurposeVp,
          },
          `negative-transport-purpose-${runId}`,
        ),
      [422],
      "transport_purpose_substitution",
      "presentation_purpose_mismatch",
    );
  }
  ...
  const associationTrace = input.exchange.lastResponseTrace;
  expect(associationTrace?.status).toBe(200);
  ...
  const associationReplayTrace = input.exchange.lastResponseTrace;
  expect(associationReplayTrace?.status).toBe(200);
  ...
  const recoveryTrace = restartedExchange.lastResponseTrace;
  expect(recoveryTrace?.status).toBe(200);
  ...
  const replayedGraph = await syncGraphToEnd({
    exchange: restartedExchange,
    contracts: input.contracts,
    state: graph.state,
  });
  expect(replayedGraph.state.nextCursor).toBe(graph.state.nextCursor);
  expect(
    replayedGraph.artifacts.map((artifact) => artifact.artifactId),
  ).toEqual(graph.artifacts.map((artifact) => artifact.artifactId));
  expect(replayedGraph.presentations).toBe(graph.presentations);
  console.info("Live Wallet P0 completion", {
    ...
    holderPresentationDigest: recovered.holderPresentationDigest,
    associationPurpose: recovered.purpose,
    associationRecipient: recovered.recipient,
    associationHttpStatus: associationTrace?.status,
    associationRequestId: associationTrace?.requestId,
    associationCorrelationId: associationTrace?.correlationId,
    associationReplayIdempotent: associationReplay.idempotent,
    associationReplayHttpStatus: associationReplayTrace?.status,
    recoveryHttpStatus: recoveryTrace?.status,
    graphReplayStable:
      replayedGraph.state.nextCursor === graph.state.nextCursor &&
      replayedGraph.presentations === graph.presentations,
    ...
    for (const change of page.changes) {
      if (change.type === "credential.upsert") {
        const prepared = await prepareWalletExchangeCredential({
          ...
          requiredRenderBlocks:
            input.contracts.renderContract.payload.requiredBlocks,
...
async function portalStaffLogin(
  openId: string,
  activeRole: "issuer_maker" | "issuer_checker" | "hospital_admin",
  ...
  const pending = (async () => {
    ...
    if (!response.ok) {
      throw tracedHttpError(
        "Portal staff test login failed",
        response,
        payload,
      );
    }
    ...
    const cookie = cookieHeader.match(
      /(?:^|[, ]+)app_session_id=([^;, ]+)/,
    )?.[1];
    if (!cookie)
      throw new Error("Portal staff login did not return a session cookie.");
    return {
      cookie: `app_session_id=${cookie}`,
      user: {
        id: positiveInteger(user.id, "Portal staff user ID"),
        openId: String(user.openId ?? ""),
        hospitalId: positiveInteger(
          user.hospitalId,
          "Portal staff hospital ID",
        ),
      },
    };
  })();
...
async function portalTrpc<T>(
  ...
  if (procedure === "trustGraph.decideTask") {
    console.info("Live Wallet Portal tRPC decision trace", {
      procedure,
      status: response.status,
      requestId: response.headers.get("x-request-id") ?? undefined,
      correlationId: response.headers.get("x-correlation-id") ?? undefined,
...
async function approveShlOrganizationTask(input: {
  hospitalId: number;
  descriptor: ShlApprovalDescriptor;
  runId: string;
}): Promise<Record<string, unknown>> {
  const admin = await portalStaffLogin("demo-hospadmin-001", "hospital_admin");
  if (admin.user.hospitalId !== input.hospitalId) {
    throw new Error(
      "Portal hospital administrator belongs to a different hospital.",
    );
  }
  const queue = await portalTrpc<unknown[]>(
    admin,
    "trustGraph.queue",
    "query",
    {
  ...
  const queueDescriptors = queue.flatMap(shlQueueDescriptors);
  const matchingDescriptors = queueDescriptors.filter(
    (candidate) =>
      candidate.taskId === input.descriptor.taskId &&
      candidate.workflowId === input.descriptor.workflowId &&
      candidate.artifactId === input.descriptor.artifactId &&
      candidate.contentHash === input.descriptor.contentHash &&
      candidate.issuerHospitalId === input.descriptor.issuerHospitalId &&
      candidate.assignedRole === input.descriptor.assignedRole,
  );
  const item = matchingDescriptors[0];
  if (!item) {
    throw new Error(
      `Portal SHL organization approval descriptor was not visible as an exact trustGraph.queue match: ${JSON.stringify({ claim: input.descriptor, queue: queueDescriptors })}`,
    );
  }
  expect(matchingDescriptors).toHaveLength(1);
  console.info("Live Wallet Certified SHL queue descriptor", item);
  return portalTrpc<Record<string, unknown>>(
    admin,
    "trustGraph.decideTask",
    "mutation",
    {
      taskId: input.descriptor.taskId,
      decision: "approve",
      reason:
        "Hospital administrator approved the exact Wallet Certified SHL digest.",
      expectedContentHash: input.descriptor.contentHash,
      correlationId: `wallet-p0-shl-organization-${input.runId}`,
    },
  );
}

type ShlApprovalDescriptor = Readonly<{
  taskId: string;
  workflowId: string;
  artifactId: string;
...
function shlApprovalDescriptor(value: unknown): ShlApprovalDescriptor {
  const descriptor = record(value);
  return {
    taskId: nonEmptyString(descriptor.taskId, "approvalTask.taskId"),
    workflowId: nonEmptyString(
      descriptor.workflowId,
      "approvalTask.workflowId",
    ),
    artifactId: nonEmptyString(
      descriptor.artifactId,
      "approvalTask.artifactId",
    ),
    issuerHospitalId: positiveInteger(
      descriptor.issuerHospitalId,
      "approvalTask.issuerHospitalId",
    ),
    contentHash: nonEmptyString(
      descriptor.contentHash,
      "approvalTask.contentHash",
    ),
    assignedRole: nonEmptyString(
      descriptor.assignedRole,
      "approvalTask.assignedRole",
    ),
  };
}

function shlQueueDescriptors(value: unknown): ShlApprovalDescriptor[] {
  const item = record(value);
  const workflow = record(item.workflow);
  const artifact = record(item.artifact);
  if (!Array.isArray(item.tasks)) return [];
  return item.tasks.map((value) => {
    const task = record(value);
    return {
      taskId: String(task.taskId ?? ""),
      workflowId: String(workflow.workflowId ?? task.workflowId ?? ""),
      artifactId: String(workflow.artifactId ?? artifact.artifactId ?? ""),
      issuerHospitalId: Number(
        workflow.issuerHospitalId ?? artifact.issuerHospitalId,
      ),
...
async function pollCredentialRequestReady(
  ...
  let lastRecoveryProblem: WalletExchangeProblemError | undefined;
  for (let attempt = 0; attempt < 20; attempt += 1) {
    let status: Awaited<
      ReturnType<WalletExchangeV2Client["getCredentialRequestStatus"]>
    >;
    try {
      status = await exchange.getCredentialRequestStatus(requestId);
    } catch (reason) {
      if (
        reason instanceof WalletExchangeProblemError &&
        reason.status === 409 &&
        reason.code === "shl_certification_issuance_incomplete"
      ) {
        lastRecoveryProblem = reason;
        console.info("Live Wallet Certified SHL recovery pending", {
  ...
  if (lastRecoveryProblem) throw lastRecoveryProblem;
  throw new Error(
    `Portal credential request ${requestId} did not become ready.`,
  );
}
...
async function expectWalletProblem(
  ...
  if (!problem || !expectedStatuses.includes(problem.status ?? 0)) {
    throw new Error(
      [
        `${label} did not fail with the expected Portal problem.`,
        problem
          ? `actualStatus=${problem.status ?? "unknown"} actualCode=${problem.code ?? "unknown"} requestId=${problem.requestId ?? "missing"} correlationId=${problem.correlationId ?? "missing"}`
  ...
  expect(problem.contentType).toBe("application/problem+json");
  if (expectedCode) {
...
function alterCompactJwt(jwt: string): string {
  ...
  if (segments.length !== 3) {
    throw new Error("Compact JWS must contain exactly three segments.");
  }
  const signature = base64url.decode(segments[2]!);
  if (signature.length === 0) {
    throw new Error("Compact JWS signature must not be empty.");
  }
...
async function resignHolderVpPurpose(input: {
  identity: HolderSigningIdentity;
  payload: unknown;
  purpose: string;
}): Promise<string> {
  const payload = { ...record(input.payload), purpose: input.purpose };
