...
describe("syncTrustCarePortalWallet", () => {
  it("uses Portal demo-login and imports only VC-backed wallet cards", async () => {
                ...
                identity_and_access: [
                  {
                    ...
                    credentialData: {
                      ...
                      issuer: { id: "did:web:trustcare.network:hospital:tcc", name: "TrustCare Central Hospital" },
    ...
    expect(calls[0]?.url).toBe("https://trustcarehealth.live/api/auth/demo-login");
    expect(calls[1]?.url).toContain("https://trustcarehealth.live/api/trpc/wallet.cardsByCategory");
