...
export function CredentialDocument({
  ...
  const { credential, subject, patient, hospital, document, issuer } =
    renderData;
  const issuerNameTh =
  ...
  if (!isIdentityDocument) {
    return (
      <article
        className={`${compact ? "credential-doc credential-doc-compact" : "credential-doc"} hospital-document document-${documentVariant(card.cardType)}`}
  ...
  return (
    <article
      className={`${compact ? "credential-doc credential-doc-compact" : "credential-doc"} medical-document document-${documentVariant(card.cardType)}`}
      style={{ "--doc-accent": accent } as CSSProperties}
    >
...
function renderDocumentBody(
  ...
  switch (card.cardType) {
    ...
    case "diagnostic_report":
      return (
        <DiagnosticReportSection report={diagnosticReportPayload(subject)} />
      );
    case "prescription":
      return (
        <MedicationSection
          title="รายการยาในใบสั่งยา"
          items={prescriptionItems(subject)}
        />
      );
    case "medication_summary":
      return (
        <MedicationSection
          ...
          items={medicationSummaryItems(subject)}
        />
      );
    case "pharmacy_dispense":
      return (
        <MedicationSection
          title="รายการจ่ายยา"
          items={pharmacyDispenseItems(subject)}
        />
      );
    case "allergy_alert":
      return (
        <AllergySection
          items={allergyItems(subject)}
    ...
    case "patient_summary":
      return (
        <ClinicalSummarySection summary={clinicalSummaryPayload(subject)} />
      );
    ...
    case "discharge_summary":
      return (
        <DischargeSummarySection summary={dischargeSummaryPayload(subject)} />
      );
    ...
    case "visa_support_letter":
      return (
        <VisaSupportLetterSection letter={visaSupportLetterPayload(subject)} />
      );
    case "guarantee_letter":
      return (
        <GuaranteeLetterSection letter={guaranteeLetterPayload(subject)} />
      );
    case "shl_manifest":
  ...
  const fields: Record<string, Field[]> = {
    patient_identity: [
      ...
      { label: "กรุ๊ปเลือด", value: getText(subject, "bloodType") },
      {
        ...
        value: displayName(getObject(subject, "emergencyContact")),
      },
    ],
    staff_identity: [
      {
      ...
      {
        label: "อีเมล",
        value: getText(subject, "email") ?? getText(patient, "email"),
      },
    ...
    insurance_eligibility: [
      {
        label: "ผู้รับประกัน",
        value:
          displayName(getObject(coverage, "payer")) ??
          getText(coverage, "payer"),
      },
      ...
      {
        label: "ต้อง pre-auth",
        value: getText(coverage, "preAuthorizationRequired"),
      },
      {
        label: "ตรวจสอบล่าสุด",
    ...
    discharge_summary: [
      {
        label: "เลขที่ Admit",
        value:
          getText(discharge, "admissionNo") ??
          getText(discharge, "encounterNo"),
      },
      {
        label: "วันที่ Admit",
        value: formatDate(getText(discharge, "admissionDate")),
      },
      {
        label: "วันที่จำหน่าย",
        value: formatDate(getText(discharge, "dischargeDate")),
      },
      {
        label: "วินิจฉัยหลัก",
        value:
    ...
    consent_receipt: [
      ...
      {
        label: "ขอบเขต",
        value: getNested(consent, ["scope"]) ?? getNested(consent, ["scopes"]),
      },
      {
        label: "หมดอายุ",
        value: formatDateTime(getText(consent, "expiresAt")),
    ...
    mpi_link_certificate: [
      ...
      {
        label: "ความเชื่อมั่น",
        value: getText(mpi, "confidence") ?? getText(mpi, "linkConfidence"),
      },
      {
        label: "นโยบายจับคู่",
        value: getText(mpi, "matchingPolicy") ?? getText(mpi, "matchAlgorithm"),
      },
      {
        label: "ตรวจทานโดย",
        value: getText(mpi, "reviewedBy") ?? getText(mpi, "linkedBy"),
      },
    ],
    travel_document_verification: [
      {
      ...
      {
        label: "ประเภท Visa",
        value: getText(travel, "visaTypeTh") ?? getText(travel, "visaType"),
      },
      {
        label: "วันหมดอายุ Passport",
        value: formatDate(getText(travel, "expiryDate")),
      },
    ...
    visa_support_letter: [
      {
        label: "เลขที่จดหมาย",
        value: getText(visa, "letterNo") ?? getText(visa, "documentNo"),
      },
      {
        label: "วัตถุประสงค์",
        value: getText(visa, "purposeTh") ?? getText(visa, "purpose"),
      },
      ...
      {
        label: "แพทย์ผู้รับผิดชอบ",
        value: displayName(getObject(visa, "responsiblePhysician")),
      },
    ],
    guarantee_letter: [
      {
        label: "เลขที่ Guarantee",
        value:
          getText(guarantee, "guaranteeNo") ??
          getText(guarantee, "guaranteeRef"),
      },
      {
        label: "Payer",
        value:
      ...
      {
        label: "ใช้ได้ถึง",
        value: formatDate(getText(guarantee, "validUntil")),
      },
    ],
    quotation: [
      {
      ...
      {
        label: "ใช้ได้",
        value: getText(quotation, "validForDays")
    ...
    claim_package: [
      ...
      },
      {
        label: "สถานะ",
        value:
          getText(claimPackage, "status") ??
          getText(claimPackage, "claimStatus"),
      },
    ],
    claim_receipt: [
      {
        label: "เลขที่ใบเสร็จ",
        value: getText(receipt, "receiptNo") ?? getText(receipt, "invoiceNo"),
      },
      ...
      {
        label: "สถานะ",
        value: getText(receipt, "status") ?? getText(receipt, "paymentStatus"),
      },
    ],
    sync_receipt: [
      {
    ...
    appointment: [
      ...
      { label: "สถานที่", value: getText(appointment, "location") },
      {
        ...
        value: getNested(appointment, ["requiredDocuments"]),
      },
    ],
    shl_manifest: [
      {
...
function documentNarrative(
  card: WalletCard,
  subject: Record<string, unknown>,
  patient: Record<string, unknown>,
): { title: string; body: string; sections: string[]; sourceSystem?: string } {
  const humanDocument = getObject(subject, "humanDocument");
  const renderData = getObject(humanDocument, "renderData") ?? humanDocument;
  const patientName =
    getText(patient, "fullNameTh") ??
