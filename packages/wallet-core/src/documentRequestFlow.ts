...
export type DocumentRequestPlan = {
  ...
  trustPolicy: "issuer_signed" | "patient_provided_unverified" | "trustcare_certified";
  warnings: string[];
  nextSteps: string[];
};
...
const sourceLabels: Record<DocumentRequestSource, Omit<DocumentRequestOption<DocumentRequestSource>, "id" | "enabled">> = {
  trustcare_portal: {
  ...
  connected_fhir: {
    ...
    description: "ดึง DocumentReference, Bundle หรือ clinical resources จากระบบต้นทาง",
  },
  payer: {
  ...
  patient_upload: {
    ...
    description: "รับ PDF, รูปภาพ หรือ FHIR JSON เป็นเอกสารอ้างอิงที่ยังไม่ยืนยัน",
  },
  external_wallet: {
...
const formatLabels: Record<DocumentRequestFormat, Omit<DocumentRequestOption<DocumentRequestFormat>, "id" | "enabled">> = {
  vc_vp: {
    label: "VC/VP",
    description: "เอกสารเดี่ยวหรือชุดเล็กที่ต้องตรวจลายเซ็นและใช้ selective disclosure",
  },
  oid4vci_offer: {
    label: "OID4VCI Offer",
    description: "ให้ issuer ส่ง credential offer เพื่อให้ Wallet รับ VC เข้ามาโดยตรง",
  },
  fhir_document_reference: {
  ...
  fhir_bundle: {
    ...
    description: "เหมาะกับชุดข้อมูลจาก HIS/EMR/LIS ที่มีหลาย resource หรือ Composition",
  },
  standard_shl: {
    label: "Standard SHL",
    description: "ลิงก์สุขภาพตาม SMART Health Links สำหรับชุดข้อมูลขนาดใหญ่หรือข้อมูลต่อเนื่อง",
  },
  certified_shl_manifest: {
    label: "SHL + Manifest VP",
    description: "ใช้ SHL เป็น transport และเพิ่ม Manifest VP/Holder VC สำหรับ TrustCare verifier",
  },
  pdf_image: {
    label: "PDF / Image Upload",
    description: "ไฟล์จากผู้ใช้หรือระบบภายนอก ต้องรอ trusted issuer ยืนยันก่อนใช้เป็น VC",
  },
};

const returnChannelLabels: Record<DocumentRequestReturnChannel, Omit<DocumentRequestOption<DocumentRequestReturnChannel>, "id" | "enabled">> = {
  portal_sync: {
  ...
  fhir_pull: {
    ...
    description: "ใช้ consent/scope ที่กำหนดเพื่อดึง DocumentReference หรือ Bundle",
  },
  shl_link: {
    label: "รับเป็น SHL",
    description: "รับ shlink หรือ viewer URL พร้อมนโยบาย PIN/expiry แยกจากตัว QR",
  },
  manual_upload: {
    label: "นำเข้าไฟล์เอง",
    description: "ผู้ใช้เลือกไฟล์หรือวาง JSON แล้วระบบเก็บเป็น DocumentReference",
  },
  external_wallet: {
...
export function buildDocumentRequestPlan(input: DocumentRequestPlanInput): DocumentRequestPlan {
  ...
  const defaultScope = selectedTypes.length > 1 ? "document_bundle" : "single_document";
  const scope = input.scope ?? defaultScope;
  const defaultSource = chooseDefaultSource(selectedRequirements);
  const sourceOptions = buildSourceOptions(selectedRequirements).map((option) => ({
    ...option,
    recommended: option.id === defaultSource,
  }));
  const requestedSource = input.source ?? defaultSource;
  const source = sourceOptions.find((option) => option.id === requestedSource && option.enabled)
    ? requestedSource
    : defaultSource;
  const defaultFormat = chooseDefaultFormat(source, scope, selectedTypes);
  const formatOptions = buildFormatOptions(source, scope, selectedTypes).map((option) => ({
    ...option,
    recommended: option.id === defaultFormat,
  }));
  const requestedFormat = input.format ?? defaultFormat;
  const format = formatOptions.find((option) => option.id === requestedFormat && option.enabled)
    ? requestedFormat
    : defaultFormat;
  ...
  const serviceLabel = canonicalServiceProfiles[input.context]?.label ?? input.context;
  return {
...
export function createDocumentRequestDraft(input: {
  ...
  const defaultReturnChannel =
    plan.returnChannelOptions.find((option) => option.enabled && option.recommended)?.id ??
  ...
  return {
    ...
    requestedRequirementKeys: plan.selectedRequirements.map((requirement) => requirement.key),
...
export function documentRequestSourceLabel(source: DocumentRequestSource): string {
  return sourceLabels[source].label;
}

export function documentRequestFormatLabel(format: DocumentRequestFormat): string {
  return formatLabels[format].label;
}

export function documentRequestReturnChannelLabel(channel: DocumentRequestReturnChannel): string {
...
function chooseDefaultSource(requirements: DocumentRequestRequirement[]): DocumentRequestSource {
  const types = requirements.flatMap((requirement) => requirement.documentTypes);
  const hints = requirements.map((requirement) => requirement.sourceHint ?? "").join(" ").toLowerCase();
  if (types.some((type) => type.includes("insurance") || type.includes("claim") || type.includes("guarantee"))) {
    return "payer";
  }
  if (hints.includes("lis") || hints.includes("ris") || hints.includes("fhir") || hints.includes("emr")) {
...
function chooseDefaultFormat(
  ...
  if (scope === "document_bundle" && source === "trustcare_portal") return "certified_shl_manifest";
  if (scope === "document_bundle" && source !== "payer") return "standard_shl";
  if (source === "connected_fhir" && types.some((type) => type === "lab_result" || type === "diagnostic_report")) {
    return "fhir_bundle";
  }
...
function buildSourceOptions(requirements: DocumentRequestRequirement[]): DocumentRequestOption<DocumentRequestSource>[] {
  const types = requirements.flatMap((requirement) => requirement.documentTypes);
  const needsPayer = types.some((type) => type.includes("insurance") || type.includes("claim") || type.includes("guarantee"));
  return (Object.keys(sourceLabels) as DocumentRequestSource[]).map((id) => {
    ...
    return {
      ...
      ...sourceLabels[id],
      enabled,
      reasonDisabled: enabled ? undefined : "ใช้เมื่อเอกสารเกี่ยวกับสิทธิ ประกัน เคลม หรือผู้จ่ายเงิน",
    };
  });
}
...
function formatDisabledReason(
  ...
  const singleIdentityOnly = scope === "single_document" && types.length === 1 && types[0] === "patient_identity";
  ...
  if (format === "standard_shl" && (source === "patient_upload" || singleIdentityOnly)) {
...
function buildReturnChannelOptions(
  ...
  return (Object.keys(returnChannelLabels) as DocumentRequestReturnChannel[]).map((id) => {
    ...
    return {
      ...
      reasonDisabled: enabled ? undefined : "ช่องทางนี้ไม่ตรงกับ source หรือ format ที่เลือก",
    };
  });
}
...
function returnChannelEnabled(
  ...
  if (channel === "fhir_pull") return format === "fhir_document_reference" || format === "fhir_bundle";
  if (channel === "shl_link") return format === "standard_shl" || format === "certified_shl_manifest";
  if (channel === "manual_upload") return source === "patient_upload" || format === "pdf_image";
  if (channel === "external_wallet") return source === "external_wallet";
  return false;
}

function buildControlState(source: DocumentRequestSource, format: DocumentRequestFormat) {
  return {
    selectiveDisclosure: format === "vc_vp" || format === "oid4vci_offer",
    fhirEndpoint: format === "fhir_document_reference" || format === "fhir_bundle",
    shlAccessPolicy: format === "standard_shl" || format === "certified_shl_manifest",
    trustCareCertification: source === "trustcare_portal" && format === "certified_shl_manifest",
    manualFileUpload: source === "patient_upload" || format === "pdf_image",
  };
}

function buildTrustPolicy(source: DocumentRequestSource, format: DocumentRequestFormat): DocumentRequestPlan["trustPolicy"] {
  if (source === "patient_upload" || format === "pdf_image") return "patient_provided_unverified";
...
function buildWarnings(
  ...
  if (source === "patient_upload") {
    warnings.push("เอกสารจากผู้ใช้จะถูกเก็บเป็น DocumentReference แบบยังไม่ยืนยัน จนกว่า trusted issuer จะตรวจและลงนาม");
  }
  if (format === "standard_shl") {
    warnings.push("Standard SHL ใช้ร่วมกับระบบภายนอกได้ แต่ยังไม่ถือเป็น TrustCare-certified จนกว่าจะมี Manifest VP และ Holder VC");
  }
  if (format === "certified_shl_manifest") {
    warnings.push("Certified SHL ต้องเก็บ Manifest VP/Holder VC และ hash ของไฟล์ไว้ให้ verifier ตรวจสอบได้");
  }
  if (format === "fhir_bundle" && scope === "document_bundle") {
    warnings.push("FHIR Bundle ควรเรียงข้อมูลตาม record time และเก็บ package time แยกเพื่อไม่ให้ timeline สับสน");
  }
  return warnings;
}

function buildNextSteps(source: DocumentRequestSource, format: DocumentRequestFormat): string[] {
  if (source === "patient_upload" || format === "pdf_image") {
    return ["ผู้ใช้เลือกไฟล์หรือวาง JSON", "Wallet สร้าง DocumentReference", "สถานะเป็นยังไม่ยืนยันจนกว่า issuer จะลงนาม"];
  }
  if (format === "certified_shl_manifest") {
    return ["ส่งคำขอไป TrustCare Portal", "Portal สร้าง SHL manifest และ Manifest VP", "Wallet sync กลับเข้ามาเป็น SHL+Manifest package"];
  }
  if (format === "standard_shl") {
    return ["ส่งคำขอไปแหล่งข้อมูล", "รับ shlink หรือ viewer URL", "Wallet ตรวจ manifest และเก็บ access policy"];
  }
  if (format === "oid4vci_offer") {
    return ["Issuer สร้าง credential offer", "Wallet รับ credential ผ่าน OID4VCI", "ตรวจลายเซ็นและสถานะก่อนใช้"];
  }
  if (format === "fhir_document_reference" || format === "fhir_bundle") {
    return ["เลือก FHIR endpoint และ scope", "ดึง DocumentReference/Bundle", "เก็บเป็น evidence และรอออก VC หากต้องใช้เป็นเอกสารรับรอง"];
  }
  return ["ส่งคำขอไป issuer", "รับ VC/VP ที่ลงนามแล้ว", "ตรวจ trust/status ก่อนนำไปใช้"];
}
