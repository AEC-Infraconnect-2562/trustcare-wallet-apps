...
export function documentReferenceFromCard(card: WalletCard): FhirDocumentReferenceLike {
  const credential = objectValue(card.credentialData);
  const subject = objectValue(credential?.credentialSubject);
  const existing = objectValue(subject?.documentReference) ?? documentReferenceFromEvidence(credential);
  if (isDocumentReference(existing)) return normalizeDocumentReference(existing, card);
  return createDocumentReference(card);
}

export function documentReferenceFromEvidence(credentialData: unknown): Record<string, unknown> | null {
...
export function validateDocumentReference(value: unknown): MhdValidationResult {
  ...
  if (!document) {
    return { ok: false, warnings, errors: ["DocumentReference must be an object."] };
  }
  if (document.resourceType !== "DocumentReference") errors.push("resourceType must be DocumentReference.");
  if (typeof document.id !== "string" || !document.id) errors.push("DocumentReference.id is required.");
  if (typeof document.status !== "string" || !document.status) errors.push("DocumentReference.status is required.");
  if (!Array.isArray(document.content) || document.content.length === 0) {
    errors.push("DocumentReference.content must contain at least one attachment.");
  } else {
    const missingContentType = document.content.some(item => !objectValue(objectValue(item)?.attachment)?.contentType);
    if (missingContentType) errors.push("Each DocumentReference.content attachment must include contentType.");
  }
  if (!document.type) warnings.push("DocumentReference.type is recommended for MHD/IPS classification.");
  if (!document.subject) warnings.push("DocumentReference.subject is recommended for patient-scoped wallet records.");
...
export function recordFromMhdDocumentReference(
  ...
  if (!validation.ok) {
    throw new Error(`Invalid MHD DocumentReference: ${validation.errors.join("; ")}`);
  }
  ...
  return {
    ...
    lifecycleStatus: documentReference.status === "current" ? "active" : documentReference.status,
    status: documentReference.status === "current" ? "active" : documentReference.status,
    ...
    source: {
      ...
      mhdDocumentReferenceUrl: documentReference.content[0]?.attachment.url ?? null,
    ...
    version: {
      ...
      documentDate,
      clinicalPeriod: objectValue(documentReference.context)?.period as { start?: string; end?: string } | undefined
    },
    privacy: {
...
function createDocumentReference(card: WalletCard): FhirDocumentReferenceLike {
  return {
    ...
    subject: card.holderDid || card.patientId ? { reference: `Patient/${card.patientId ?? card.holderDid}` } : undefined,
    date: card.issuedAt ?? card.createdAt,
    author: card.issuerHospitalName ? [{ display: card.issuerHospitalName }] : undefined,
    custodian: card.issuerDid ? { reference: card.issuerDid } : undefined,
    content: [
      {
        attachment: {
          contentType: card.credentialJwt ? "application/vc+jwt" : "application/vc+json",
...
function normalizeDocumentReference(value: Record<string, unknown>, card: WalletCard): FhirDocumentReferenceLike {
  const existingContent = Array.isArray(value.content) ? value.content : [];
  return {
    ...value,
    resourceType: "DocumentReference",
    id: typeof value.id === "string" && value.id ? value.id : String(card.credentialId),
    status: typeof value.status === "string" && value.status ? value.status : "current",
    docStatus: typeof value.docStatus === "string" ? value.docStatus : "final",
    type: objectValue(value.type) ?? {
      text: card.displayName
    },
    category: Array.isArray(value.category) ? value.category as Array<Record<string, unknown>> : [{ text: card.documentCategory }],
    content: existingContent.length
      ? existingContent as FhirDocumentReferenceLike["content"]
      : createDocumentReference(card).content,
    date: typeof value.date === "string" ? value.date : card.issuedAt ?? card.createdAt
  };
}
