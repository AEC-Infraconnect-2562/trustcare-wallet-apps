...
- `gatewayMode`: `portal_backend`, `static_demo_gateway`, หรือ `local_preview`
- `storageProvider`: `s3`, `static`, หรือ `local`
- `manifestEndpointMethod`: `POST`, `GET`, หรือ `BOTH`
- `trustLayerStatus`: `standard_shl`, `pending_manifest_vp`, หรือ `certified_manifest_vp`
- `documentBundle.documents[]`: รายการเอกสารที่ผูกกับ FHIR DocumentReference และ SHL files
- `portalRequest`: request contract สำหรับ debug/developer mode เท่านั้น ไม่ควรแสดงให้ผู้ใช้ทั่วไป
