...
## Portal Issuer Authority

Portal/Railway และ KMS เป็น authority ของ issuer DID และ signing key สำหรับ
TCC, TCP และ TCM. Wallet ไม่มี DID/JWKS registry สำรองและไม่ derive DID จาก
hostname. `portalIssuerResolver` ต้องอ่าน DID document กับ JWKS จาก Portal
discovery/Trust Registry แล้ว cross-check issuer, hospital code, controller,
active assertion method และ public key ทุกครั้งก่อนยอมรับ credential.

Credential เก่าหรือ credential ใดที่ issuer ไม่ตรงกับ discovery ปัจจุบันต้อง
quarantine/reject และขอให้ Portal reissue; Wallet ห้าม re-sign เพื่อกลบ proof
หรือ issuer ที่ขาดหาย.

## Implementation Boundaries

- `walletExchangeV2.ts` เป็น HTTP/session/DPoP contract เท่านั้น
- `walletExchangeWorkflow.ts` เป็น orchestration, retry, cursor และ ACK workflow
- `walletExchangePersistencePolicy.ts` เป็นกติกา atomic persistence ร่วมกัน
- Web IndexedDB และ Mobile SQLite เป็น platform adapters แยกกัน
- `holderPresentation.ts` สร้างและ sign VP ใหม่ต่อ sharing event
- `credentialRenderer.ts` เป็น authoritative renderer facade; domain helpers
  แยกเป็น renderer types, physical form, identity, primitives และ payload
  normalization โดยไม่สร้าง renderer ชุดที่สอง
- `CredentialDocument.tsx` เป็น Web presentation ของ renderer กลาง

ห้ามสร้าง legacy Portal sync repository, presentation builder หรือ renderer
อีกชุดกลับมา. Web/Mobile ต้องเรียก shared workflow/domain contract เหล่านี้.
...
- `CertifiedSHLPackage`: `holder_attested` SHL ที่ได้รับ Manifest Credential แบบ `application/vc+jwt` จาก Portal/KMS และตรวจครบแล้ว
...
ผู้ใช้อื่น หรือใช้ demo record เป็น silent production fallback.
...
- ถ้าต้องแชร์ ให้ route ไป Share เพื่อสร้าง `PurposeVP`, `StandardSHL` หรือ `CertifiedSHLPackage` ตามขนาดข้อมูลและ trust policy
...
- `holder_attested` ใช้แชร์ได้ทันทีในระดับ Standard SHL แต่ห้ามแสดงว่าโรงพยาบาลรับรองแล้ว
- ห้ามแสดงเป็น hospital-certified จนกว่า holder VP, Manifest Credential, issuer/kid/signature/status/expiry, subject, hashes, purpose, audience และ policy จะตรวจครบ
...
Local development ใช้ Vite in-memory gateway ที่ `/api/share-gateway` เพื่อทดสอบ flow เดียวกันโดยไม่ฝัง payload ลง QR และเปิด `/api/share-gateway/.well-known/jwks.json` ให้ verifier ตรวจลายเซ็นจริง. Production Share Gateway ต้องเป็น server-side resolver ที่มี durable database และ persistent signing JWK/KMS; private key และ service token ห้ามอยู่ใน browser bundle.

## Demo Resolver

GitHub Pages ไม่มี backend สำหรับ enforce passcode, expiry, revocation, access count และ audit. Static resolver จึงเป็น synthetic demo fixture เท่านั้นและห้ามเป็น fallback ของ sandbox/pilot/production runtime.

- SHL manifest ใช้ `tc_resolver=shl-manifest`
- Holder VP และ Manifest Credential ใช้ resolver URL ที่คืน JWT content type ตรงชนิดเท่านั้น
- Legacy VP `tc_payload` รองรับเฉพาะ backward compatibility และห้ามใช้ให้ green badge

Production ต้องใช้ Share Gateway endpoint ที่ discovery คืนมาและตรวจ durable artifact, expiry, access policy และ trust evidence ตาม `docs/SHL_GATEWAY_ARCHITECTURE.md`.
...
Store แสดง portable objects ได้แก่ VC, holder VP, SHL, hospital Manifest Credential, OID4VCI offers, OID4VP requests และ scan/import history
