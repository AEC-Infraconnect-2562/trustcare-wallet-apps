...
| A4 | **Faking (D2)** — walk-in demo คืน `did:key:walkin-<ts>` ที่ไม่ใช่ did:key จริง | `wallet.ts:connectWalkInWallet` ใน `usesDemoRuntime` | คืน `holderDid: null` + `holderReference: urn:trustcare:demo-walkin:<id>` — walk-in ที่ยังไม่ผูกไม่มี holder key; did:key จริง mint ตอน link เท่านั้น |
...
| ~~D2~~ | **แก้แล้ว** — walk-in demo เคยคืน `did:key:walkin-${Date.now()}` (ไม่ใช่ did:key จริง) | `wallet.ts:connectWalkInWallet` | เลือกทางที่ซื่อสัตย์ที่สุด: walk-in ที่ยัง `ready_to_link`/`pending_consent` **ยังไม่ผูก holder key** จึงคืน `holderDid: null` + `holderReference: urn:trustcare:demo-walkin:<id>` — ไม่แสร้งเป็น did:key (การปั้น did:key จริงแล้วทิ้ง private key ก็ยังหลอกว่าผูกได้) did:key จริงถูก mint ตอน link wallet เท่านั้น |
