...
export function StoreScreen() {
  ...
  const objects = useMemo(
    () => mergeWalletObjects(baseObjects, cached),
    [baseObjects, cached],
  );
  ...
  useEffect(() => {
    ...
    void loadStoredObjects(user.id)
      .then(setCached)
      .catch(() => undefined);
  }, [baseObjects, user.id]);
  ...
  const importPayload = async () => {
    ...
    setMessage(
      result.ok ? `นำเข้า ${result.format} แล้ว` : result.errors.join(", "),
    );
    setPayload("");
  };
  ...
  return (
    <ScrollView style={styles.root} contentContainerStyle={styles.content}>
      ...
      <View style={styles.importPanel}>
        ...
        />
        <Pressable
          style={[styles.importButton, !payload.trim() && styles.disabled]}
          disabled={!payload.trim()}
          onPress={() => void importPayload()}
        >
          <FileJson color="#fff" />
        ...
        <View key={object.id} style={styles.objectCard}>
          <View style={styles.objectTop}>
            <View style={styles.badgeRow}>
              ...
              {!!object.protocol && (
                <Text style={styles.protocol}>{object.protocol}</Text>
              )}
            </View>
            <ShieldCheck color={toneColor(storedObjectTone(object))} />
          </View>
          <Text style={styles.objectTitle}>{object.title}</Text>
          <Text style={styles.objectSub} numberOfLines={2}>
            {object.subtitle ?? object.source ?? object.id}
          </Text>
