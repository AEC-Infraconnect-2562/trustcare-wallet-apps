...
export function SettingsScreen() {
  const { user, users, setActiveUserId, resetActiveUser } =
    useActiveWalletUser();
    ...
    <ScrollView style={styles.root} contentContainerStyle={styles.content}>
      ...
      <View style={styles.userPanel}>
        ...
        <Text style={styles.desc}>
          เลือกผู้ใช้โดยไม่ต้องใช้รหัสผ่านในช่วงพัฒนา ข้อมูลใน Wallet จะแยกตาม
          scope ของผู้ใช้ที่เลือก
        </Text>
        {users.map((item) => {
          ...
          return (
            <Pressable
              ...
              onPress={() => void setActiveUserId(item.id)}
            >
              ...
              <View style={styles.userCopy}>
                ...
                <Text style={styles.userMeta}>
                  {item.role === "staff" ? "เจ้าหน้าที่" : "ผู้ป่วย"} ·{" "}
                  {item.sourceLabel}
                </Text>
              </View>
      ...
      <View style={styles.row}>
        <View style={styles.copy}>
          <Text style={styles.name}>Biometric gate</Text>
          <Text style={styles.desc}>
            ยืนยันตัวตนก่อนแสดง QR และรายละเอียดสำคัญ
          </Text>
        </View>
        <Switch
          value={security.biometricEnabled}
          onValueChange={(enabled) =>
            void security.setBiometricEnabled(enabled)
          }
        />
      </View>
      <View style={styles.row}>
        <View style={styles.copy}>
          <Text style={styles.name}>ป้องกันการบันทึกหน้าจอ</Text>
          <Text style={styles.desc}>
            ลดความเสี่ยงจาก screenshot บนหน้าข้อมูลสำคัญ
          </Text>
        </View>
        <Switch
          value={security.screenCaptureProtectionEnabled}
          onValueChange={(enabled) =>
            void security.setScreenCaptureProtectionEnabled(enabled)
          }
        />
      </View>
      <View style={styles.row}>
        <View style={styles.copy}>
          <Text style={styles.name}>ภาษา</Text>
          <Text style={styles.desc}>
            สลับภาษา UI ระหว่างไทยและอังกฤษสำหรับหน้าที่ใช้ i18n
          </Text>
        </View>
        <Pressable
      ...
      <View style={styles.row}>
        <View style={styles.copy}>
          <Text style={styles.name}>ธีม</Text>
          <Text style={styles.desc}>
            บันทึกโหมดธีมและใช้กับ navigation surface ของ mobile wallet
          </Text>
        </View>
        <Pressable
          style={styles.segmentButton}
          onPress={() =>
            void security.setTheme(
              security.theme === "light" ? "dark" : "light",
            )
          }
        >
          <Text style={styles.segmentText}>
            {security.theme === "light" ? "LIGHT" : "DARK"}
          </Text>
        </Pressable>
      </View>
      <Pressable
      ...
      <View style={styles.note}>
        <Shield color="#0b6b42" />
        <Text style={styles.desc}>
          Mobile app ใช้ SecureStore สำหรับ refresh token และ SQLite สำหรับ
          wallet cache ในเครื่อง
        </Text>
      </View>
    </ScrollView>
  );
